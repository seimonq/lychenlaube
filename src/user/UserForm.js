...
export default class UserForm extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.state = {user: new User("Simon","simon@kern.de","admin",true,
        new UserData(2,new Map([[2022,10],[2021,11]])))}
  }

  render() {
    return (<Paper elevation={3} variant={"outlined"} sx={{margin:5,width:.4}}>
      <TextField id="user-name" label="name" value={this.state.user.name} variant="outlined"  sx={{margin: 2}} InputProps={{
        readOnly: true,
      }}/>
      <br/>
